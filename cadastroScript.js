...
var cssNavbar = document.querySelector('.navbar');
var cssNavbarActive = document.querySelector('.navbarActive');
var cssNavbarInactive = document.getElementsByClassName('navbarInactive');
...
//#region navbar
cssNavbar.style.margin = '0';
cssNavbar.style.padding = '0';
cssNavbar.style.listStyleType = 'none';
cssNavbar.style.overflow = 'hidden';
cssNavbar.style.backgroundColor = colorGray3;
cssNavbarActive.style.borderBottom = '2px solid' + colorBlue1;

for (var i = 0; i < cssNavbarInactive.length; i++){
    cssNavbarInactive[i].style.color = colorWhite1;
    cssNavbarInactive[i].style.float = 'left';
    cssNavbarInactive[i].style.color = 'left';
    cssNavbarInactive[i].style.display = 'block';
    cssNavbarInactive[i].style.textAlign = 'center';
    cssNavbarInactive[i].style.padding = '14px 16px';
    cssNavbarInactive[i].style.textDecoration = 'none';
}

function NavbarMouseOver0(){
    for (var i = 0; i < cssNavbarInactive.length; i++){
        cssNavbarInactive[0].style.backgroundColor = colorGray2;
    }
}
function NavbarMouseLeave0(){
    for (var i = 0; i < cssNavbarInactive.length; i++){
        cssNavbarInactive[0].style.backgroundColor = 'transparent';
    }
}
function NavbarMouseOver1(){
    for (var i = 0; i < cssNavbarInactive.length; i++){
        cssNavbarInactive[1].style.backgroundColor = colorGray2;
    }
}
function NavbarMouseLeave1(){
    for (var i = 0; i < cssNavbarInactive.length; i++){
        cssNavbarInactive[1].style.backgroundColor = 'transparent';;
    }
}
function NavbarMouseOver2(){
    for (var i = 0; i < cssNavbarInactive.length; i++){
        cssNavbarInactive[2].style.backgroundColor = colorGray2;
    }
}
function NavbarMouseLeave2(){
    for (var i = 0; i < cssNavbarInactive.length; i++){
        cssNavbarInactive[2].style.backgroundColor = 'transparent';;
    }
}
//#endregion
...
for (var i = 0; i < cssFormsLabel.length; i++) {
    ...
    cssFormsLabel[2].style.paddingRight = '14.5em';
...
function Responsividade(screenSize) {
    if (window.innerWidth <= 700) {
        ...
        for (var i = 0; i < cssFormsLabel.length; i++) {
            ...
            cssFormsLabel[2].style.paddingRight = '11em';
